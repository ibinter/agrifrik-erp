...
const kpis = [
  ...
  { label: "Catégories", value: "4", sub: "agricole Â· intrants Â· matières Â· piscicole", icon: BarChart2, color: "#1565C0", bg: "#E3F2FD" },
  { label: "Alertes rupture", value: "2", sub: "action requise immédiate", icon: AlertTriangle, color: "#B71C1C", bg: "#FFEBEE" },
  { label: "Rotation stock", value: "4,2Ã-/trim", sub: "vs 3,8Ã- trim précédent", icon: RotateCcw, color: "#6A1B9A", bg: "#F3E5F5" },
];
...
export default function StocksPage() {
  ...
  const displayRows = rows.length > 0 ? rows : STOCKS;
  ...
  async function handleModalSubmit(fields?: { produit?: string; unite?: string; quantite?: string }) {
    if (fields) {
      await dbPost("stocks", {
        nom: fields.produit ?? "",
        unite: fields.unite ?? "",
        quantite: parseFloat(fields.quantite ?? "0"),
        quantite_min: 0,
        statut: "Normal",
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {displayRows.map((s, idx) => {
                const produit = String((s as Record<string, unknown>).produit ?? (s as Record<string, unknown>).nom ?? "");
                const categorie = String((s as Record<string, unknown>).categorie ?? "");
                const quantite = Number((s as Record<string, unknown>).quantite ?? 0);
                const quantite_min = Number((s as Record<string, unknown>).quantite_min ?? (s as Record<string, unknown>).seuilCritique ?? 0);
                const unite = String((s as Record<string, unknown>).unite ?? "");
                const entrepot = String((s as Record<string, unknown>).entrepot ?? "");
                const valeur = Number((s as Record<string, unknown>).valeur ?? 0);
                const statut = String((s as Record<string, unknown>).statut ?? "Normal");
                const critique = statut === "Bas" || quantite < quantite_min;
                const key = String((s as Record<string, unknown>).id ?? idx);
                return (
                  <tr key={key} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3 text-xs font-medium text-gray-900 whitespace-nowrap">{produit}</td>
                    <td className="px-4 py-3 text-xs text-gray-500 whitespace-nowrap">{categorie}</td>
                    <td className="px-4 py-3 text-xs font-bold text-gray-900">{quantite.toLocaleString()}</td>
                    <td className="px-4 py-3 text-xs text-gray-500">{unite}</td>
                    <td className="px-4 py-3 text-xs text-gray-500 whitespace-nowrap">{entrepot}</td>
                    <td className="px-4 py-3 text-xs text-gray-700 whitespace-nowrap">{valeur.toLocaleString()}</td>
