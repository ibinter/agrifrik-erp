...
export default async function GrandLivrePage({ params }: Props) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      <Topbar
        title={`Grand livre — Compte ${id}`}
        breadcrumb={["Finance", "Comptabilité", id]}
      />
      ...
      <main className="flex-1 p-6 max-w-6xl mx-auto w-full space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-[#2E7D32]/10 flex items-center justify-center">
            <span className="text-[#2E7D32] font-bold text-sm">{id}</span>
          </div>
          <div>
            <p className="font-semibold text-gray-900">Compte {id} — Ventes de produits agricoles</p>
