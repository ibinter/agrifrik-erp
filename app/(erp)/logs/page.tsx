...
// --- Donnees logs ---
const LOGS: LogEntry[] = [
  { date: "11/07 09h14", user: "Ibrahim S.", module: "Messagerie", action: "MESSAGE_SENT", objet: "Equipe EXP-001", ip: "154.0.x.x" },
  { date: "11/07 08h47", user: "Systeme", module: "Meteo", action: "DATA_REFRESH", objet: "AWS-EXP001 (auto)", ip: "Serveur" },
  { date: "11/07 08h32", user: "Koffi Amani", module: "Auth", action: "LOGIN", objet: "admin@agrifrik.com", ip: "102.176.x.x" },
  { date: "11/07 08h15", user: "Koffi Amani", module: "Messagerie", action: "MESSAGE_SENT", objet: "Equipe EXP-001", ip: "102.176.x.x" },
  { date: "11/07 07h00", user: "Ibrahim S.", module: "Auth", action: "LOGIN", objet: "ibrahim.s@agrifrik.com", ip: "154.0.x.x" },
  { date: "11/07 06h00", user: "Systeme", module: "Alertes", action: "ALERT_TRIGGERED", objet: "Cut test LOT-047", ip: "Serveur" },
  { date: "11/07 06h00", user: "Systeme", module: "Alertes", action: "ALERT_TRIGGERED", objet: "Stock KCl critique", ip: "Serveur" },
  ...
  { date: "10/07 16h22", user: "Adjoua M.", module: "Tresorerie", action: "RECORD_CREATED", objet: "MVT-2025-0921", ip: "102.176.x.x" },
  { date: "10/07 16h18", user: "Adjoua M.", module: "Comptabilite", action: "RECORD_CREATED", objet: "JNL-2025-0921", ip: "102.176.x.x" },
  { date: "10/07 14h30", user: "Koffi Amani", module: "Achats", action: "ORDER_CREATED", objet: "ACH-2025-024 KCl", ip: "102.176.x.x" },
  { date: "10/07 11h22", user: "Ibrahim S.", module: "Planning", action: "TASK_UPDATED", objet: "PCT-2025-034 -> Planifie", ip: "154.0.x.x" },
  { date: "09/07 15h14", user: "Koffi Amani", module: "Devis", action: "RECORD_CREATED", objet: "DEV-2025-003 OLAM", ip: "102.176.x.x" },
  { date: "08/07 14h23", user: "Systeme", module: "Tresorerie", action: "AUTO_RECONCILED", objet: "MVT-2025-0921", ip: "Serveur" },
  { date: "08/07 08h00", user: "Koffi Amani", module: "Stocks", action: "ALERT_RESOLVED", objet: "Super Cupravit livre", ip: "102.176.x.x" },
  { date: "08/07 08h00", user: "Systeme", module: "Alertes", action: "ALERT_REFRESHED", objet: "7 alertes verifiees", ip: "Serveur" },
  ...
  { date: "05/07 18h01", user: "Systeme", module: "Rapports", action: "REPORT_SENT", objet: "Rapport CA hebdo", ip: "Serveur" },
  { date: "04/07 09h22", user: "Koffi Amani", module: "Ventes", action: "RECORD_UPDATED", objet: "VNT-2025-008 -> Reglee", ip: "102.176.x.x" },
  ...
  { date: "01/07 14h18", user: "Ibrahim S.", module: "Qualite", action: "CQ_VALIDATED", objet: "CQ-2025-046 Grade AA OK", ip: "154.0.x.x" },
];
...
function DonutUsers() {
  ...
  const data = [
    ...
    { label: "Systeme (auto)", pct: 4, color: "#9ca3af" },
...
function ActionBadge({ action, flagged }: { action: ActionType; flagged?: boolean }) {
  if (flagged || action === "LOGIN_FAILED") {
    return <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-red-100 text-red-700">ALERTE {action}</span>;
  }
...
// --- Filtres ---
type FilterType = "Tous" | "Creations" | "Modifications" | "Suppressions" | "Connexions";
const FILTERS: FilterType[] = ["Tous", "Creations", "Modifications", "Suppressions", "Connexions"];

function matchFilter(log: LogEntry, filter: FilterType): boolean {
  if (filter === "Tous") return true;
  if (filter === "Creations") return log.action.includes("CREATED") || log.action.includes("SUBMITTED") || log.action.includes("UPLOADED") || log.action === "ORDER_CREATED";
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-5">
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">Audit trail complet - Tracabilite de toutes les operations AGRIFRIK</p>
            </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          />
          <span className="text-xs text-gray-500">{filtered.length} entree{filtered.length > 1 ? "s" : ""}</span>
        </div>
                ...
                {visible.map((log, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-gray-500 font-mono text-[11px] whitespace-nowrap">
                      {log.flagged ? (
                        <span className="text-red-600 font-semibold">{log.ip} [ALERTE]</span>
                      ) : (
                        log.ip
                      )}
                    </td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="rounded-2xl border border-gray-100 bg-white p-5">
            <h2 className="text-sm font-semibold text-gray-800 mb-1">Activite par utilisateur</h2>
            <p className="text-[11px] text-gray-400 mb-4">30 derniers jours</p>
            <DonutUsers />
          </div>
        </div>
